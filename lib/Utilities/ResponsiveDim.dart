import 'package:get/get.dart';
class ResponsiveDim{
  static double screenHeight=Get.context!.height;
  static double screenWidth=Get.context!.width;

  static double pageViewContainer = screenHeight/4.04;
  static double pageView = screenHeight/2.78;
  static double pageViewTextContainer = screenHeight/6.35;

  static double height10= screenHeight/89.02;
  static double height15=screenHeight/59.3523;
  static double height20= screenHeight/44.51;
  static double height30= screenHeight/29.67;
  static double height45= screenHeight/19.78;
  static double height35= screenHeight/25.43;
  static double height310=screenHeight/2.871;
  static double height250=screenHeight/3.56;
  static double height5=screenHeight/173.485;


  //390.428571429
  static double width10= screenHeight/89.02;
  static double width15=screenHeight/59.3523;
  static double width20= screenHeight/44.51;
  static double width30= screenHeight/29.67;
  static double width45= screenHeight/19.78;
  static double width5= screenHeight/178.0571;
  static double width140= screenHeight/6.359;
  static double width145= screenHeight/  6.139;
  static double width150= screenHeight/  5.9352;
  static double width390=screenHeight/2.2802;



  static double bigFont=screenHeight/35.61;
  static double smallFont=screenHeight/59.352;
  static double font24=screenHeight/37.0952;

  static double radius20=screenHeight/44.51;
  static double radius30=screenHeight/29.67;
  static double radius15= screenHeight/59.3523;
  static double radius6= screenHeight/148.3809;
  static double radius8=screenHeight/108.42857;


  static double icon24= screenHeight/37.09;
  static double icon44= screenHeight/20.2337;






}
